...
const decodeAppendSequencerBatch = (b) => {
    const calldata = '0x' + FOUR_BYTE_APPEND_SEQUENCER_BATCH.toString('hex') + common_1.remove0x(b);
...
exports.sequencerBatch = {
    ...
    decode: (b) => {
        const buf = Buffer.from(common_1.remove0x(b), 'hex');
...
class BatchedTx extends bufio_1.Struct {
    ...
    getSize() {
        ...
        const tx = transactions_1.serialize({
            nonce: this.tx.nonce,
        ...
        this.raw = Buffer.from(common_1.remove0x(tx), 'hex');
        return this.raw.length + 3;
    }
    write(bw) {
    ...
    toTransaction() {
        ...
        return transactions_1.parse(this.raw);
    }
    toHexTransaction() {
        if (this.raw) {
    ...
    toJSON() {
        if (!this.tx) {
            this.tx = transactions_1.parse(this.raw);
        }
        return {
            nonce: this.tx.nonce,
    ...
    fromHex(s, extra) {
        const buffer = Buffer.from(common_1.remove0x(s), 'hex');
